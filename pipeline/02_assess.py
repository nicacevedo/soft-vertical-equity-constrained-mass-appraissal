...
This stage runs **two parallel selection rules** over the existing CV
...
2. ``nash`` — **Nash equilibrium** (product-of-utilities / log-sum) selection
   ...
   ``LGBCovPenalty`` only (``--nash-families``).
...
updated with the two ``config_id`` winners so subsequent stages can read them
...
def main() -> None:
    p = argparse.ArgumentParser(description="Assess stage — two-path model selection over CV artifacts.")
    ...
    p.add_argument(
        ...
        help="Comma-separated model families for the Nash selector (default: LGBCovPenalty only).",
    )
    p.add_argument(
        "--utopia-families",
        type=str,
    ...
    print("=" * 70)
    print("ASSESS — two-path model selection")
    print("=" * 70)
