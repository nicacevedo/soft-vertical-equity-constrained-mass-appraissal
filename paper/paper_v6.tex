...
\todo{Before treating any reported $R^2_P$ value as final, verify that the implementation uses the evaluation-sample mean $\bar P_S$ exactly as defined in Eq.~\eqref{eq:predictive_metrics}. If a training-mean-relative score is retained for implementation diagnostics, give it a different name and do not report it as $R^2_P$. Regenerate the baseline table after this check.}
...
\todo{Before treating Table~\ref{tab:ccao_baseline_results} as final, verify that $R^2_P$ is computed exactly as defined in Eq.~\eqref{eq:predictive_metrics} and that MKI uses the price-ordered concentration index in Eq.~\eqref{eq:mki_components}. Regenerate the table and the interpretation paragraph if either implementation changes.}
...
\todo{Regenerate Figure~\ref{fig:baseline_motivation} before final submission so that its labels match the manuscript terminology: replace ``Assessment ratio'' with ``Valuation-to-sale ratio,'' replace ``Test'' with ``Held-out evaluation,'' replace ``2025'' with ``2025 forward evaluation,'' and remove the additional generic ``Slope'' annotation. Retain only $\beta_{\log}$, which is defined in Eq.~\eqref{eq:log_ratio_slope}. Do not change the underlying data, bins, curves, or $\beta_{\log}$ values.}
...
\todo{The metrics subsection states that the full VEI percentile-group profile with 90\% confidence intervals is reported. Once that proxy-based figure is generated from the final metric implementation, place it in the Results or Appendix unless it adds information beyond Table~\ref{tab:ccao_baseline_results} that is needed in this motivation subsection. It need not be a second live baseline figure here.}
...
{Section~\ref{sec:empirical_design} specifies the chronological validation and penalty-selection procedure used in the empirical application. Appendix~\ref{app:theory} provides complementary intuition for the direct penalty: within a fixed linear prediction space, increasing $\rho$ shrinks the targeted covariance smoothly while the increase in squared error is second order near $\rho=0$. This result is an interpretive benchmark, not a guarantee for fully retrained boosted trees, whose splits, leaves, and stopping behavior can change with the penalty.}
