...
% TODOs remain in the source for the empirical freeze but are hidden by default.
% Remove `disable' only when preparing an internal audit copy.
...
% ---------------------------------------------------------------------------
% TIER-A DOCUMENT-INTEGRITY OVERRIDE (P0.1; audit P0-8 / P0-13).
% Neutralises the revision-tracking macros defined above for the reader-facing
% build: accepted text renders as ordinary prose, and text this manuscript had
% ALREADY marked as deleted stops printing. No call site is edited, so the whole
% change is reverted by deleting this block. Every suppressed passage is archived
% verbatim in paper_analysis/paper_v17/superseded_text_archive.md.
% Re-run paper_analysis/paper_v17/markup_preflight.py before altering this block.
\renewcommand{\newtext}[1]{#1}
\renewcommand{\latesttext}[1]{#1}
\renewcommand{\advisorchange}[1]{#1}
\renewcommand{\oldtext}[1]{}
\renewenvironment{oldrevisionblock}{\setbox0\vbox\bgroup}{\egroup}
% Some passages apply the revision colours directly via \textcolor{impGreen}{..}
% or {\color{latestPurple} ..} rather than through the macros above. Redefining
% the two colours to black neutralises every such site without editing any of
% them. To restore the tracked-changes view, put back the RGB values above
% (impGreen 0,128,0 and latestPurple 128,0,128) and delete this block.
\definecolor{impGreen}{RGB}{0,0,0}
\definecolor{latestPurple}{RGB}{0,0,0}
% ---------------------------------------------------------------------------
...
% Superseded title: Fair Property Taxation: A Covariance-Guided Correction for Assessor Workflows
\title{\latesttext{Covariance-Based Regularization for Regressivity in Machine-Learning Real Estate Valuation}}
...
% The superseded abstract remains in the source history above.  This revision uses
% one self-contained abstract so that scope, estimand, implementation, and evidence
% can be read without consulting tracked fragments.
\latesttext{Machine-learning real estate valuation models can predict property sale prices with high accuracy, while still exhibiting systematic biases  or unfair patterns. One of the most common equity problems in this setting is that of \textit{regressivity}; a type of price-related bias. We study a deliberately narrow intervention for that problem in a research translation of the Cook County Assessor's Office (CCAO) residential LightGBM workflow. Understanding regressivity as an undesired dependence between log-price residuals and the actual sale prices, we add either (i) a direct squared-covariance penalty between that residual and log sale price or (ii) a sample-additive Jensen upper bound that is a log price distance-weighted squared-error objective. The direct LightGBM implementation supplies the exact squared-covariance gradient but only the diagonal of its dense rank-one Hessian, while the Surrogate is observation-additive which permits both the gradient and Hessian to be implemented exactly. Seven rolling-origin folds, a later held-out block, and a separate 2025 forward sample trace the complete---fixed base learner--- regularization paths rather than just blindly selecting a deployment penalty. Across considerable portions of both regularization paths, the targeted first-order dependence and several assessor-facing vertical-equity diagnostics move toward neutrality before stronger regularization produces predictive and ratio-shape deterioration. The full paths also show that first-order neutrality does not imply a flat conditional ratio profile, local equity, or residual--price independence. Because sale price is both outcome and a market value proxy, the penalty controls a price-related pattern among observed sales; that is, it does not identify inequity relative to latent market value or final tax burdens. The contribution is therefore a CCAO-motivated objective translation, an exact distinction between Direct and Surrogate mechanisms, and a temporal failure-mode audit of their attainable paths---not a general fairness guarantee or a selected operational model.}
...
{We address this challenge by modifying the valuation model's training objective. Specifically, we use the global first-order dependency between log valuation errors and log sale prices as a penalization mechanism: we formulate a direct squared-covariance penalty and also develop a sample-additive upper-bound surrogate that is compatible with LightGBM's custom objective interface. For a fixed penalty formulation, the correction changes only the training objective and introduces a single additional tuning parameter: the penalty strength. It requires no changes to the underlying data, feature set, or surrounding validation and evaluation workflow. 
% \oldtext{For a later deployment decision, chronological validation can first be used to derive a family-specific \emph{candidate region} that screens the positive-$\rho$ path before any final operating-point rule is applied. The lower endpoint is only a soft activity-onset marker, while the upper endpoint is a family-specific guardrail against the first robust high-$\rho$ deterioration or nonlinear-structure warning. This screen narrows the configurations passed to downstream selection; it does not select $\rho$, define a universally ``safe'' interval, or imply that raw-$\rho$ endpoints are temporally invariant.}
{We use chronological validation to characterize each complete regularization path; a secondary screen removes configurations with little detectable activity or clear high-penalty deterioration, but it neither ranks the remaining models nor selects the penalty weight.} The framework is intentionally targeted: it does not, by itself, determine overall valuation level, eliminate nonlinear or local inequity, or replace the assessor-facing vertical-equity diagnostics used for evaluation.}
...
{In the primary CCAO application, model development and the complete penalty paths are evaluated chronologically within the development sample, followed by a primary held-out evaluation and a separate 2025 forward evaluation after refitting\todo{This part sounds redundant still}. \latesttext{The baseline level reported in Section~\ref{subsec:baseline_tension} establishes the application-specific motivation on its own: the current model class attains high predictive accuracy on the price scale and still exhibits a substantial regressive vertical-equity pattern among observed sales. No comparison with another model class is needed for that, and none is reported.}
% \oldtext{This draft reports the complete Direct and Surrogate regularization paths}
{We report the complete direct and surrogate regularization paths} without choosing a penalty strength.}
...
% The preceding green contribution paragraph is retained above as source history;
% the active contribution statement below replaces it.
{The paper makes three application-driven contributions. First, it translates a CCAO ratio-study concern into the explicit first-order dependency of log ratios and log prices target 
% $\operatorname{Cov}(\log(\widehat P/P),\log P)^2$ 
and implements its regularization path inside a research translation of the office's residential LightGBM workflow. Second, it separates two objectives that can otherwise be conflated: the direct penalty has sample-coupled rank-one curvature, whereas its Jensen surrogate is exactly weighted squared error with weights increasing in squared log-price distance from the training center\todo{also redundant with previous story}. Fixed prediction-space results expose the direct centered-spread geometry and the Surrogate's generally multi-directional weighted-projection geometry. Third, it evaluates complete fixed base learner paths under rolling-origin validation, held-out testing, and a forward year, using ratio-study, mechanism, nonlinear-shape, and residual-dependence diagnostics to document both attainable improvements and failure modes. Covariance penalties and dependence-based fair regression are established ideas; the novelty claimed here is their precise mass-appraisal translation, the direct vs surrogate distinction, and the CCAO-centered temporal path audit.}
% \oldtext{We report complete regularization paths rather than a selected penalty.}
\newtext{Throughout the primary CCAO analysis, we report the complete regularization paths rather than select a deployment operating point.} \todo{This is also unclear or redundant}
...
\latesttext{Accordingly, we do not claim novelty for covariance regularization \cite{KomiyamaEtAl2018,TrederEtAl2021}, fairness-aware empirical-risk minimization, or the general idea of trading prediction loss against an application-specific criterion. Nor is a global slope correction the only way to address regressivity: segmented valuation, richer spatial and temporal information, and post-hoc affine recalibration \cite{BeheshtiEtAl2019} are close alternatives. The narrower methodological contribution is to formulate the exact log-ratio/log-price target used here, derive and distinguish its Direct and observation-additive Surrogate objectives, and evaluate their full temporal paths in a CCAO-motivated mass-appraisal setting. This positioning makes the empirical comparison against a centered post-hoc spread map scientifically necessary before any claim that in-processing is preferable; the present paper does not make that comparative claim.}
...
Nothing in the following list is novel here: residual--outcome decorrelation itself;
covariance or correlation control as a training criterion; a tunable decorrelation path
indexed by a penalty strength; affine post-hoc orthogonalization of residuals against the
outcome; target-dependent corrective losses; and nonlinear in-processing correction. The
contribution is narrower, and specific to mass appraisal: the CCAO-centered translation of
the problem into the assessor's own diagnostics; the exact first-order identification of
the training target with the classical log-ratio/log-price vertical-equity slope; a
LightGBM-compatible Direct implementation together with an accurate account of the
curvature structure its custom-objective interface admits; a Jensen-derived,
sample-additive Surrogate; the finding that Direct and Surrogate are diagnostically
distinct rather than two implementations of one mechanism; a joint audit of the
first-order slope, the nonlinearity gap and distance correlation; a chronological and
forward-in-time validation that reports its failure modes explicitly rather than
summarizing them as a pass; and an interpretation situated in the assessment workflow
rather than in model selection. No penalty strength is selected anywhere in this paper.
...
& $1$
& $[0.95,1.05]$
& \newtext{May 2026 (exposure draft)}
...
\newtext{The guidance-status column records that standing explicitly, because
...
\latesttext{The same fixed estimator was later applied, fold by fold, to the already-frozen chronological-validation predictions as a retrospective diagnostic; those reconstructed CV values do not define the $\rho$ grid or any model-selection rule.}
...
\latesttext{The level is itself the empirical motivation. On the primary held-out evaluation the
...
\latesttext{Table~\ref{tab:ccao_baseline_results} and Figure~\ref{fig:baseline_motivation} present the primary baseline level in the main text. Appendix Table~\ref{tab:ccao_baseline_complementary} reports the complementary valuation-level, horizontal-uniformity, and mechanism/residual-structure diagnostics for the same cell.}
...
\newtext{The Jensen step is what makes the objective observation-additive, and it is not
...
\newtext{The Direct objective's curvature was audited directly, and the audit does not
...
\newtext{What can be stated exactly is the structure. The Hessian of the Direct
...
\latesttext{For comparisons across training samples, the raw numerical value of $\rho$ is not intrinsically portable. Under a rescaling $y\mapsto ay$, squared error scales as $a^2$ while $C(f)^2$ scales as $a^4$; the scale-invariant Direct coordinate is therefore}
\begin{equation}
\label{eq:normalized_rho}
\latesttext{\widetilde\rho=\rho\,\widehat{\Var}_{\mathrm{train}}(y).}
\end{equation}
\latesttext{Raw $\rho$ remains the coordinate of the CCAO implementation reported below. Any cross-jurisdiction comparison must recompute the training-block variance without leakage and report $\widetilde\rho$; even then, normalization is a comparability device, not a universal operating rule.}
...
\newtext{The extract is identified by content rather than by a version string. Every
...
\end{center}
\newtext{as recorded in the frozen preflight provenance. Two limitations belong
...
\paragraph{\newtext{Development coordinates.}}
\newtext{Three development coordinates appear below, and they are not interchangeable.
...
\newtext{The three differ because of a property of the fold construction that has to be
...
\newtext{D1's status under that overlap is exactly five things, and the last two are the
...
\latesttext{No deployment operating point is selected in the present analysis.} \newtext{The chronological-CV path is used only to characterize the path.}
...
Penalty path & \latesttext{82 positive values per family: original 50-point $[0.1,100]$ path plus a 32-point lower-tail extension $[0.00110,0.08685]$} \\
Current analysis & \latesttext{Full CV, held-out, and forward paths; descriptive path landmarks only; no screening result and no deployment operating-point selection} \\
\bottomrule
\end{tabularx}
\end{table}
...
\newtext{The empirical comparison in this paper covers the baseline model class, the custom-objective implementation control, and the Direct and Surrogate training-time regularization paths,} \latesttext{together with a global post-hoc centered rescaling of the same unregularized predictors. That comparator is reported in Sections~\ref{subsec:posthoc_comparator} and~\ref{subsec:matched_beta_results}, both descriptively and at matched development first-order correction. \emph{Structured} post-hoc calibration by geography, property class or time is a different construction and remains outside the empirical scope.}

\paragraph{\newtext{Reference cells, and what each one licenses.}}
\newtext{Three reference cells appear below, and the distinction between two of them is
...
\newtext{The attribution rule that goes with them is not a nicety. Comparisons against
...
\latesttext{The primary CCAO Results report the complete Direct and Surrogate regularization paths. The fixed cross-fitted correlation-ratio nonlinearity gap $\Delta_{\mathrm{NL}}$ is traced along those paths: the held-out and 2025 values are the original out-of-time diagnostics, while the chronological-CV values are reconstructed from the already-frozen validation predictions using the same fixed estimator. The artifact that reproduces the positive-$\rho$ path anchors carries no $\Delta_{\mathrm{NL}}$ column, so no positive-$\rho$ value of it is reported for these paths, no screening result is derived from it, and no deployment operating point is selected.}
...
\latesttext{One implementation qualification applies to every headline path comparison, and Section~\ref{subsec:zero_reference_result} states it as a rule rather than a caveat. In the frozen CCAO artifacts the two custom-objective fits at $\rho=0$ agree with each other but are not numerically identical to ordinary LightGBM, despite algebraically matching squared-error derivatives. Positive-$\rho$ changes relative to the within-family $\rho=0$ origin describe the effect of regularization along that custom-objective training path; differences relative to ordinary LightGBM also carry the native-versus-custom execution-path difference. Appendix~\ref{subsec:zero_control_results} reports the audit.}

\subsection{Accuracy Alone Does Not Remove the CCAO Ratio Trend}
\label{subsec:ccao_baseline_results}

\latesttext{Section~\ref{subsec:baseline_tension} reports the level of the assessor-facing
...
\subsection{\latesttext{The Zero-Penalty Reference Cells and What They Permit}}
\label{subsec:zero_reference_result}

\latesttext{Every path statement below is read against one of two reference cells, and the
...
\latesttext{Three findings from the frozen zero-penalty audit fix how those cells may be used. First,
...
\latesttext{The consequence is a rule rather than a caveat. A difference between ordinary LightGBM
...
% \oldtext{What the Prespecified Regularization Paths Do}
\latesttext{What the Final Augmented Regularization Paths Show}}{What the Final Augmented Regularization Paths Show}}
...
\latesttext{The compact main-text view is split in two, because the provenance of its two
...
\caption{\latesttext{Regularization path at the two frozen positive-$\rho$ anchors: prediction, PRD
...
\scriptsize
\emph{Notes.}
\latesttext{The two rows under \textit{Reference cells} are the two frozen reference roles, and they
...
\newtext{At the nominal $\rho=1$ display anchor (tested $\rho\approx 0.954095$) both families improve
...
log-scale cost.}
% \oldtext{Given the current implementation audit, within-family path changes are read relative to the custom-objective rho=0 control.}
...
\renewcommand{\arraystretch}{1.08}
\caption{\latesttext{Standards-facing vertical equity at all five nominal display anchors, with the
...
\scriptsize
\emph{Notes.}
\latesttext{PRB is the price-related bias measure of the adopted 2013 Standard on Ratio Studies, whose
...
\latesttext{The anchor rows in Tables~\ref{tab:path_anchor_frozen}
...
range before recovering at higher prices. \newtext{This visual contrast is descriptive rather than a structural guarantee: Direct covariance controls the affine residual--price component but does not protect against nonlinear conditional-mean departures at stronger regularization. The ratio-shape contrast here is therefore read from the profiles themselves; the two families' non-affine residual structure is compared numerically at matched first-order correction in Section~\ref{subsec:matched_beta_results}, not along these paths.} 
...
\latesttext{The fixed cross-fitted $\Delta_{\mathrm{NL}}$ nonlinearity gap is reported here only where a frozen artifact reproduces it. At the shared zero-penalty path origin it is $0.116$ on the held-out sample and $0.121$ in 2025. The frozen artifact that reproduces the positive-$\rho$ display anchors carries no $\Delta_{\mathrm{NL}}$ column, so no positive-$\rho$ value of the gap is quoted for these paths and no claim is made about how it moves along them. (The matched first-order targets reported later are a different coordinate, with their own frozen source.) The ratio-shape contrast described above is therefore read from the profiles themselves rather than from the scalar gap.}
...
\caption{\latesttext{Mechanism and residual-structure paths versus $\rho$ for Direct and Surrogate on held-out (solid) and 2025 (dashed) evaluations. The $\beta_{\log}=0$ line is a first-order neutrality reference; $\Delta_{\mathrm{NL}}$ and dCor are not given assessor-style zero targets. The shaded band spans the smallest to the largest tested $\rho$ at which any of five equal-weight cross-validation criteria---$R^2_P$, MAE, MAPE, $\operatorname{RMSE}_{\log P}$, and COD---attains its best value on the tested grid. It is descriptive only: it is derived from the chronological development folds even where it is drawn over held-out and 2025 curves, its endpoints are not reproduced by any artifact in the frozen evidence set, and it does not delimit a selected, portable, or recommended penalty interval. No penalty strength is selected in this paper.}}
...
\newtext{Along these paths the broader distance-correlation diagnostic is quoted only at the zero-penalty origin, where it is $0.382$ on the held-out sample and $0.417$ in 2025. The same frozen artifact carries no distance-correlation column either, so no positive-$\rho$ value and no claim about the shape of the dCor path are asserted for them.} 
...
\paragraph{\latesttext{The map, and the one sensitivity attached to it.}}
\latesttext{The comparator rescales a cached prediction array about the mean log \emph{target} of
...
\latesttext{A second centering --- about the fitting block's mean \emph{prediction}
...
\paragraph{\latesttext{Three roots, and they are three different objects.}}
\latesttext{The rescaling coefficient can be defined on three different samples, and collapsing them
...
\paragraph{\latesttext{The descriptive comparison, and it crosses over.}}
\latesttext{Table~\ref{tab:posthoc_crossover} places Direct's most-corrected held-out
...
\latesttext{Read in one direction the comparison favours rescaling; read in the other it favours
...
\paragraph{\latesttext{What a one-parameter rescaling costs, and what it cannot do.}}
\latesttext{Driving the development coordinate to first-order neutrality post-hoc requires
...
\latesttext{The comparator is built on the custom-objective $\rho=0$ origin because that cell is the
...
\small
\setlength{\tabcolsep}{5pt}
\caption{\latesttext{Descriptive juxtaposition of the two most-corrected retrained configurations
...
\small
\emph{Notes.}
\latesttext{$\beta_{\log}$ is the held-out first-order slope, which is what the two rows in each
...
\subsection{\latesttext{Matched First-Order Correction: Direct, Surrogate and Post-Hoc Rescaling}}
\label{subsec:matched_beta_results}

\latesttext{The comparison in Section~\ref{subsec:posthoc_comparator} is keyed on an outcome
...
\paragraph{\latesttext{The design, and the tolerance it was frozen with.}}
\latesttext{Matching uses the primary development coordinate D1, the equal-weight mean of the seven
...
\latesttext{One qualification governs every reading of the outcome columns. The configurations are
...
\paragraph{\latesttext{At the strongest common target the retrained Direct path is the worst of the
three.}}
\latesttext{Table~\ref{tab:matched_beta_headline} reports the held-out outcome at $j=5$, the
...
\paragraph{\latesttext{Price-level and log-scale accuracy disagree, and both are reported.}}
\latesttext{The extended targets in Table~\ref{tab:matched_beta_ext} make that disagreement
...
\paragraph{\latesttext{Non-attainment over the evaluated path.}}
\latesttext{Three of the extended targets lie outside the three-way common support, and the frozen
...
\paragraph{\latesttext{The one-sale-one-vote sensitivity moves a material subset of the
comparison.}}
\latesttext{Because the fold-6 and fold-7 validation blocks overlap, the primary D1 coordinate
...
\latesttext{Taken together, the three families are not interchangeable at matched first-order
...
\small
\setlength{\tabcolsep}{5pt}
\caption{\latesttext{Held-out outcome at the strongest development first-order correction all three
...
\small
\emph{Notes.}
\latesttext{C-posthoc is the centered rescaling of the \emph{Custom-objective $\rho=0$ origin}, the
...
evidence about temporal sensitivity rather than a defect to be tuned away. The shaded
landmarks retained in the appendix path figures accordingly describe where particular
development-period diagnostics begin to move, and they do not delimit a selected,
portable, or recommended operating interval.
...
a proposed sample-construction rule. It leaves the within-path conclusions in place, but
it does materially attenuate the broader-dependence rebound in the Surrogate family, and
...
% Superseded discussion opening retained in the repository diff.
\latesttext{The CCAO application answers a bounded model-design question. Along the currently frozen custom-objective paths, covariance-guided regularization can materially attenuate the regressive first-order pattern among observed sales, and moderate configurations can do so before predictive performance deteriorates. Stronger correction can alter valuation level, ratio dispersion, and nonlinear ratio structure. These are fixed-base-learner path results: the 994-tree LightGBM configuration is held constant to isolate objective behavior, so the experiment does not estimate each objective family's fully retuned attainable frontier. In addition, native-to-penalized contrasts remain partly confounded until the initialization-aligned $\rho=0$ parity rerun is propagated through the primary artifacts. The evidence therefore supports a practical control for one component of a sale-price-related ratio pattern, not a complete solution to vertical equity or a deployment model.}

The complete paths sharpen that conclusion in two ways. First, moderate
regularization need not impose an immediate accuracy cost \latesttext{relative to each family's custom-objective path origin}: parts of both
...
% Detailed candidate-region interpretation is retained in Appendix~\ref{app:ccao_path_details}.
\latesttext{The CV-derived candidate regions summarize where activity becomes visible and where a family-specific deterioration or caution signal first binds. Their numerical endpoints are descriptive outputs of the current folds and screen definition, not confidence bounds, validated selectors, or portable ``safe'' intervals. Exact event locations transfer weakly to the held-out and forward paths, so the full paths remain the primary evidence and the detailed screen belongs in the appendix.}
...
% Superseded scope paragraph retained in the repository diff.
\latesttext{The evidence also bounds the paper's claim. Better information can improve prediction and equity together. The proposed penalties target a countywide first-order pattern among observed sales; they do not determine valuation level, guarantee assessor-facing reference ranges, remove nonlinear or local inequity, establish residual--price independence, or identify effects on final tax liabilities. The active manuscript makes no external-validity claim from the superseded ATTOM analysis. Covariance-guided regularization should therefore be treated as one auditable modeling option within a broader valuation-review process, not as evidence that training-time regularization is necessary or sufficient for vertical equity.}
...
\begin{enumerate}[leftmargin=7mm]
    ...
    \item Report baseline prediction and valuation-level results. Add COD, PRD, PRB, MKI, VEI, value-group ratio profiles, $\beta_{\log}$, $\Delta_{\mathrm{NL}}$, and dCor.
    ...
    % Superseded operating-point instruction is replaced by items 6--8 below.
    \item \latesttext{Optionally use a prespecified CV screen to remove inactive or clearly deteriorated configurations. Treat its endpoints as descriptive and family-specific, not as uncertainty bounds or a validated selection rule.}
    \item \latesttext{Compare surviving in-processing configurations with constant calibration and a validation-fitted centered spread map, using identical temporal splits and diagnostics.}
    \item \latesttext{If a later operating point is required, choose it using an explicit predictive/equity loss and prespecified guardrails, then assess temporal, subgroup, and parcel-level robustness.}
...
% Superseded limitations paragraph retained in the repository diff.
\latesttext{First, the evidence is sales-based and application-specific. The primary analysis is a Python research translation of CCAO's published residential workflow, not an official production run or assessment decision. Outcomes on verified transacting properties need not transport to the full roll when sale propensity, verification, property condition, or unobserved quality differs between sold and unsold parcels. The paper therefore estimates model behavior on the observed sale distribution, not jurisdiction-wide tax incidence or inequity relative to latent market value. The active paper also makes no external-validity claim from the superseded multi-county analysis.}
...
% Superseded limitations paragraph retained in the repository diff.
\latesttext{Second, the estimand and correction are deliberately narrow. Equation~\eqref{eq:bayes_residual_covariance} shows that residual--sale-price covariance contains mechanically negative association from unpredictable transaction variation; forcing it toward zero may spread predictions toward realized sale noise. Direct targets only a global log-scale covariance, while the Surrogate changes observation weights and may also alter level and tail fit. Neither objective directly optimizes PRD, PRB, VEI, valuation level, $\Delta_{\mathrm{NL}}$, or local equity. Direct exponentiation of log predictions is an operational retransformation, not a smearing or conditional-mean correction. A countywide first-order improvement can therefore coexist with nonlinear, geographic, class-specific, temporal, or level inequity, and the reported price-scale path should be checked against smearing or other retransformation choices.}
...
% Superseded limitations paragraph retained in the repository diff.
\latesttext{Third, the experiment characterizes fixed-994-tree regularization paths, not a deployment decision or fully retuned family frontier. The current primary artifacts also require an initialization-aligned native/custom $\rho=0$ parity rerun. Apart from selected group-profile intervals, the paper reports descriptive point estimates and fold variation rather than inferential uncertainty for path differences. The chronological split is ordered by sale date, but a final robustness audit should keep equal-date transactions together and block repeated sales of the same parcel to rule out boundary and parcel-history leakage. The candidate-region screen is heuristic and data-derived: leave-one-fold-out stability is not a confidence interval, and weak transfer of exact turning locations rules out interpreting raw-$\rho$ endpoints as portable. Finally, the study omits the closest post-processing comparator---the centered spread map implied by the Direct fixed-space result---so it cannot claim that training-time regularization is necessary or superior.}
...
\newtext{Before attributing within-family changes to positive regularization, we compare ordinary LightGBM with the Direct and Surrogate implementations at $\rho=0$. The two custom-objective controls coincide with each other in the reported metrics, but the currently frozen artifacts are not numerically identical to ordinary LightGBM. Because the supplied zero-penalty gradient and Hessian match squared-error derivatives algebraically, this audit identifies a native-versus-custom execution-path difference rather than a positive-penalty objective effect. \latesttext{The paragraphs after the table report what the completed audit then established, and the boundary of that claim: the evidence isolates the setting responsible and the boosting iteration at which the two paths separate, but it does not directly instrument LightGBM's internal random-number stream, so no claim is made about library internals beyond what the tree, split and source evidence supports.}}
...
\latesttext{\paragraph{The native-versus-custom decomposition, as measured.} The parity experiment
...
\latesttext{Two further results fix the interpretation. Repeating the whole ladder with LightGBM's
...
\latesttext{\paragraph{Normalization and scaling, as audited.} The executed code has been audited
...
the frozen record, and the complete working-tree diff that generated the reported artifacts is
archived with the replication materials. No factor-of-two or sample-size convention is absorbed into
...
\renewcommand{\arraystretch}{1.06}
\newcommand{\baselinecompmetric}[1]{\hspace*{0.6em}#1}
\caption{\latesttext{Complementary baseline level for \emph{Ordinary LightGBM (standard raw-label native)}, the assessor-facing workflow benchmark: valuation level, horizontal uniformity, and mechanism/residual structure. A level statement about one model class, not a comparison between model classes.}}
...
\setlength{\tabcolsep}{3.4pt}
\renewcommand{\arraystretch}{1.08}
\caption{\latesttext{Complementary diagnostics at the two frozen positive-$\rho$ anchors: valuation
...
\scriptsize
\emph{Notes.}
\latesttext{Median, mean and weighted-mean ratio, COD and COV are the assessor-facing valuation-level
...
\latesttext{The machine-readable combined path table archived with the replication artifacts carries every family--$\rho$ configuration for the Section~\ref{subsec:metrics} metrics, together with the fold values, the equal-weight CV mean and standard deviation, and the held-out and 2025 forward results. Chronological-CV $\Delta_{\mathrm{NL}}$ was reconstructed fold by fold from the already-frozen validation predictions using the same fixed cross-fitted estimator specified in Appendix~\ref{app:metrics}; those CV values are archived separately and did not define the $\rho$ grid or any model-selection rule. Split-specific $\rho=0$ prediction-parity audits are stored in the same result root.}


% Historical v1--v3 transition-provenance paragraphs are preserved in git history.
...
\latesttext{The $\rho$-evolution figures below trace the frozen tested grid descriptively. They mark no penalty interval, and this paper does not select, recommend, or certify a penalty strength.}
...
\caption{\latesttext{Predictive-metric and valuation-level/uniformity paths along the Direct and Surrogate grids, with held-out and 2025 evaluations overlaid. Valuation-level panels include the ratio $=1$ reference. The shaded band spans the smallest to the largest tested $\rho$ at which any of five equal-weight cross-validation criteria---$R^2_P$, MAE, MAPE, $\operatorname{RMSE}_{\log P}$, and COD---attains its best value on the tested grid. It is descriptive only: it is derived from the chronological development folds even where it is drawn over held-out and 2025 curves, its endpoints are not reproduced by any artifact in the frozen evidence set, and it does not delimit a selected, portable, or recommended penalty interval. No penalty strength is selected in this paper.}}
...
\caption{\latesttext{Vertical-equity metric paths versus $\rho$ for Direct and Surrogate. Reference lines mark PRD $=1$, PRB $=0$, MKI $=1$, and VEI $=0$. The shaded band spans the smallest to the largest tested $\rho$ at which any of five equal-weight cross-validation criteria---$R^2_P$, MAE, MAPE, $\operatorname{RMSE}_{\log P}$, and COD---attains its best value on the tested grid. It is descriptive only: it is derived from the chronological development folds even where it is drawn over held-out and 2025 curves, its endpoints are not reproduced by any artifact in the frozen evidence set, and it does not delimit a selected, portable, or recommended penalty interval. No penalty strength is selected in this paper.}}
...
\caption{\latesttext{Chronological-fold predictive-metric paths (thin gray) and equal-weight CV means (thick). The seven folds are overlapping cumulative-window constructions rather than independent replications, so their spread is descriptive and supports no standard error. The shaded band spans the smallest to the largest tested $\rho$ at which any of five equal-weight cross-validation criteria---$R^2_P$, MAE, MAPE, $\operatorname{RMSE}_{\log P}$, and COD---attains its best value on the tested grid. It is descriptive only: it is derived from the chronological development folds even where it is drawn over held-out and 2025 curves, its endpoints are not reproduced by any artifact in the frozen evidence set, and it does not delimit a selected, portable, or recommended penalty interval. No penalty strength is selected in this paper.}}
...
\caption{\latesttext{Chronological-fold valuation-level and uniformity paths (thin gray) and equal-weight CV means (thick). Valuation-level panels include the ratio $=1$ reference. The seven folds are overlapping cumulative-window constructions rather than independent replications, so their spread is descriptive and supports no standard error. The shaded band spans the smallest to the largest tested $\rho$ at which any of five equal-weight cross-validation criteria---$R^2_P$, MAE, MAPE, $\operatorname{RMSE}_{\log P}$, and COD---attains its best value on the tested grid. It is descriptive only: it is derived from the chronological development folds even where it is drawn over held-out and 2025 curves, its endpoints are not reproduced by any artifact in the frozen evidence set, and it does not delimit a selected, portable, or recommended penalty interval. No penalty strength is selected in this paper.}}
...
\caption{\latesttext{Chronological-fold vertical-equity paths (thin gray) and equal-weight CV means (thick). Reference lines mark PRD $=1$, PRB $=0$, MKI $=1$, and VEI $=0$. The seven folds are overlapping cumulative-window constructions rather than independent replications, so their spread is descriptive and supports no standard error. The shaded band spans the smallest to the largest tested $\rho$ at which any of five equal-weight cross-validation criteria---$R^2_P$, MAE, MAPE, $\operatorname{RMSE}_{\log P}$, and COD---attains its best value on the tested grid. It is descriptive only: it is derived from the chronological development folds even where it is drawn over held-out and 2025 curves, its endpoints are not reproduced by any artifact in the frozen evidence set, and it does not delimit a selected, portable, or recommended penalty interval. No penalty strength is selected in this paper.}}
...
\caption{\latesttext{Chronological-fold mechanism paths for $\beta_{\log}$, $\Delta_{\mathrm{NL}}$, and distance correlation (thin gray) and equal-weight CV means (thick). The $\beta_{\log}=0$ line is a first-order neutrality reference; $\Delta_{\mathrm{NL}}$ and dCor are not given forced zero lines. CV $\Delta_{\mathrm{NL}}$ was reconstructed from frozen validation predictions with the same fixed estimator used out of time; it is a retrospective diagnostic and does not select $\rho$. The seven folds are overlapping cumulative-window constructions rather than independent replications, so their spread is descriptive and supports no standard error. The shaded band spans the smallest to the largest tested $\rho$ at which any of five equal-weight cross-validation criteria---$R^2_P$, MAE, MAPE, $\operatorname{RMSE}_{\log P}$, and COD---attains its best value on the tested grid. It is descriptive only: it is derived from the chronological development folds even where it is drawn over held-out and 2025 curves, its endpoints are not reproduced by any artifact in the frozen evidence set, and it does not delimit a selected, portable, or recommended penalty interval. No penalty strength is selected in this paper.}}
\label{fig:cv_mechanism_metric_paths}
\end{figure}

\latesttext{The reconstructed CV $\Delta_{\mathrm{NL}}$ paths in the figure above are a retrospective diagnostic. No minimum value and no grid location is quoted from them, because the reconstructed CV path values are not part of the frozen numeric evidence, and no location on those paths is a selected operating point.}
...
\subsection{\latesttext{Centered-Spread Post-Hoc Comparator Detail}}
\label{app:centered_spread_detail}

\latesttext{Table~\ref{tab:centered_spread} reports the comparator of
...
the root verification, and the anchor-level centering sensitivity are archived with the replication
artifacts. Two properties of the construction are worth restating here because they bound what the
...
\renewcommand{\arraystretch}{1.06}
\caption{\latesttext{Centered-spread post-hoc comparator at the frozen base-value anchors. The map
...
\scriptsize
\emph{Notes.}
\latesttext{Panels A and B rescale the \emph{Custom-objective $\rho=0$ origin}, the primary post-hoc
...
\subsection{\latesttext{Matched First-Order Targets and Attainment}}
\label{app:matched_beta_detail}

\latesttext{Tables~\ref{tab:matched_beta} and~\ref{tab:matched_beta_ext} carry the design and the
...
archived with the replication artifacts.}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.06}
\caption{\latesttext{Matched first-order comparison on the primary development coordinate D1.
...
\scriptsize
\emph{Notes.}
\latesttext{Targets are placed by a deterministic rule: probe equally spaced points in the
...
\setlength{\tabcolsep}{3.4pt}
\renewcommand{\arraystretch}{1.06}
\caption{\latesttext{Extended development targets, which lie outside the three-way common range,
...
\scriptsize
\emph{Notes.}
\latesttext{Configuration is a fitted penalty value for the Surrogate rows and an exactly solved
...
\subsection{\newtext{Retransformation Sensitivity}}
\label{app:smearing}

\newtext{Predictions are returned to the price scale by direct exponentiation,
...
\newtext{The consequences separate by construction. Valuation-level ratios---median, mean and
...
factor, and the frozen sensitivity confirms that each of them is unchanged: no vertical-equity,
uniformity, or residual-structure result in this paper depends on the retransformation choice.
...
\newtext{Table~\ref{tab:smearing_matched_beta} reports the movement at the three matched
...
$\Delta_{\mathrm{NL}}$ and dCor entries of Table~\ref{tab:matched_beta_headline} are unchanged.}

\begin{table}[!ht]
\centering
\caption{\newtext{Retransformation sensitivity at the matched first-order targets, development
...
\newtext{One asymmetry is recorded rather than resolved. The factor is estimated on development
...
\latesttext{The planned CCAO replication package should contain the complete penalty grid; fold-level, held-out, and forward predictions for every valid configuration; the fixed baseline hyperparameters and configuration hash; the native/custom $\rho=0$ parity audit; and the fixed $\Delta_{\mathrm{NL}}$ specification and values. It should also archive the original and augmented grid manifests, CV-freeze status, event and leave-one-fold-out tables, retrospective concordance and regret outputs, decade-anchor mapping, figure-QA manifests, canonical paper-asset manifest, and hashes tying all outputs to the same data, configuration, and split identity.}
...
\section{Status of the External Benchmark}
\label{app:attom}

\latesttext{The active paper does not report the superseded six-county ATTOM analysis. The current repository contains a broader protocol-frozen benchmark that uses the dimensionless coordinate $\widetilde\rho$ from Eq.~\eqref{eq:normalized_rho}, explicit zero-penalty parity checks, rolling-origin paths, and a 2025 forward evaluation. Before those results are incorporated, the benchmark's endpoint-validity rules, summary labels, bootstrap design, and native/custom parity must be audited against the raw jurisdiction-level paths. Any eventual report must describe the data as a common-model external stress test, distinguish county names from their principal cities, and keep official assessment-ratio evidence separate from AVM-generated valuation ratios. Until that audit is closed, the external project is reproducibility evidence and future robustness work, not part of the paper's inferential support.}


\section*{Data, Code, and Institutional Statement}

\advisorchange{The primary application uses CCAO data} and a research translation of the office's published residential modeling workflow. Reproducibility therefore depends on archiving the exact custom objectives, preprocessing specifications, temporal splits, prediction artifacts, metric code, and executed configuration hashes. The separate external benchmark uses licensed ATTOM-derived records that cannot be redistributed and is not part of the active paper's inferential evidence. The results reported here are research findings, not official CCAO assessment decisions. The paper makes no claim that CCAO has adopted the correction or placed it into production.



% FINAL-PASS AUTHOR CHECKLIST (remove from clean submission source)
% EMPIRICAL FREEZE BEFORE SUBMISSION: (1) propagate the initialization-aligned
% two-split rho=0 parity rerun through every primary CCAO table and figure;
% (2) run equal-date and repeated-parcel split audits; (3) compare direct
% exponentiation with a prespecified retransformation correction; (4) add
% uncertainty and subgroup/local ratio studies for any operating-point claim;
% (5) insert the exact CCAO extract/version, build date, executed configuration
% hash, and released-artifact manifest; and (6) produce a clean submission source
% with revision and TODO markup disabled. External-benchmark integration remains
% a separate audited revision.
