...
% B3.1: the interpretive hierarchy below replaces the two-point "the complete
% paths sharpen that conclusion" paragraph, the CV candidate-region summary, and
% the separate scope paragraph. All three are retained in the repository diff.

\paragraph{\latesttext{Predictive accuracy alone does not remove the price-related ratio trend.}}
\latesttext{In this application the two coexist. The ordinary workflow benchmark, \emph{Ordinary
...
\paragraph{\latesttext{The target is one global, first-order, price-related mechanism.}}
\latesttext{Both objectives control a single scalar: the covariance $\Cov(e,y)$ between the log
...
and not fairness of the final tax burden. Better information can still improve prediction and equity
together, and the penalties do not compete with that: they target a countywide first-order pattern
among observed sales, and they do not determine valuation level, guarantee assessor-facing reference
ranges, remove nonlinear or local inequity, establish residual--price independence, or identify
effects on final tax liabilities. Section~\ref{subsec:correction_scope} states the corresponding
denials for the objectives themselves, and every result below is read inside that scope.}

\paragraph{\latesttext{A one-parameter post-hoc rescaling reproduces much of the first-order
movement.}}
\latesttext{The closest post-processing alternative is not a straw comparator and is not treated as
...
\paragraph{\latesttext{Equalizing the first-order trend does not equalize the residual geometry.}}
\latesttext{The three families are nevertheless not interchangeable, and the comparison that shows
...
\paragraph{\latesttext{Part of the price-scale comparison depends on the retransformation
convention.}}
\latesttext{Predictions are returned to the price scale by direct exponentiation, which is an
...
\paragraph{\latesttext{The correction-strength relationship does not transport across market
periods.}}
\latesttext{The regularization paths describe tradeoffs over the tested grids, and that is the whole
...
\paragraph{\latesttext{What the study establishes, and where it stops.}}
...
\latesttext{Taken together, the contribution is a diagnostic and a training mechanism for assessor
workflows rather than a deployment rule. The paper establishes how to target one specific
...
% B3.1: the sequence now separates path characterization from operating-point
% selection. The superseded CV-screen and prespecified-guardrail steps are
% retained in the repository diff.
\newtext{For a later operational implementation, the correction can be incorporated into an existing AVM development workflow through the following sequence.} \latesttext{The final step is a prospective decision this study does not make.}

\begin{enumerate}[leftmargin=7mm]
    \item Build a defensible baseline model using verified sales, appropriate time adjustment, relevant property features, and local market information\latesttext{, fitted as the office would ordinarily run it. This is the workflow benchmark against which the rest is described}.
    ...
    \item \latesttext{Diagnose the first-order mechanism on that benchmark before changing the objective.} Report baseline prediction and valuation-level results. Add COD, PRD, PRB, MKI, VEI, value-group ratio profiles, $\beta_{\log}$, $\Delta_{\mathrm{NL}}$, and dCor.
    \item Fit the Direct and Surrogate covariance-guided objectives over prespecified logarithmic grids of $\rho$, including $\rho=0$ as an implementation control\latesttext{ and as the within-family origin against which penalty effects are read}.
    \item Summarize the complete regularization paths across chronological validation folds, including fold-level variability\latesttext{, and report the paths themselves rather than a preferred segment of them}.
    \item \latesttext{Compare the training-time configurations against the simple post-hoc alternatives on the same predictions --- constant level calibration and a validation-fitted centered spread map --- using identical temporal splits and diagnostics.}
    \item \latesttext{Compare families at matched first-order correction rather than at matched penalty, and read predictive accuracy on both the price and log scales alongside the assessor-facing vertical-equity measures and the broader residual-structure diagnostics. State the retransformation convention under which any price-scale comparison is made.}
...
% B3.1: the three superseded limitation paragraphs are retained in the
% repository diff. Nothing listed below is an analysis that has in fact been
% completed and reported elsewhere in the paper.
\latesttext{\emph{The target and the proxy.} Sale price is simultaneously the learning target, the
...
\latesttext{\emph{What the objective does not control.} The objective controls a global first-order
...
\latesttext{\emph{Temporal evidence and its limits.} The seven rolling-origin folds are overlapping
...
\latesttext{\emph{Scope, and the research/production boundary.} The evidence is centered on one
...
multi-county analysis. Research evaluation is also not deployment: no operational penalty strength
