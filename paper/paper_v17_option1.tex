...
Current analysis & \latesttext{Full CV, held-out, and forward paths; secondary CV screen and transition diagnostics; no deployment operating-point selection} \\
\bottomrule
\end{tabularx}
\end{table}
...
ordinary LightGBM. Aligned on sale identifiers, the two differ by a mean absolute log deviation of
3.24e-02, with a maximum of 3.09e-01, on the held-out sample, and by 3.06e-02, with a maximum of
...
The largest gap between the two centers over the fitted blocks is 8.36e-06 and it bounds the
log shift over the whole grid at 6.266e-06; the largest metric movement the alternative
...
$-0.15$, so an in-sample calibration would under-correct by roughly a factor of two in $b-1$. The
...
\latesttext{Table~\ref{tab:posthoc_crossover} places the two most-corrected retrained
configurations beside post-hoc rescalings of the custom-objective $\rho=0$ origin at a comparable
held-out first-order correction. The frozen record is explicit that this juxtaposition is keyed on
...
reproduces or improves much of the first-order accuracy and equity frontier that retraining reaches
over the range retraining attains, and it does not reproduce the residual-structure behaviour.}

\latesttext{The comparator is built on the custom-objective $\rho=0$ origin because that cell is the
common origin of both penalized families, so rescaling it holds the execution path fixed and
isolates retraining. The same map applied to \emph{Ordinary LightGBM (standard raw-label native)} is
...
block are matched on approximately; the post-hoc base value is solved on the development coordinate
and then evaluated here. Post-hoc rows are rescalings of a cached prediction array through
...
the Surrogate carries a markedly lower correlation-ratio nonlinearity gap than either comparator,
and it is the only family whose gap falls below the origin value.}
...
sits well above the same floor while pointing the other way. The two metrics weight the price distribution
differently --- price-scale $R^2_P$ is dominated by the expensive tail that the Surrogate's
$1+\rho c_i^2$ weighting protects, while $\operatorname{RMSE}_{\log P}$ weights all sales equally in
logs --- so they can order the same pair of configurations in opposite directions. Neither settles
...
13 of the 24 matched configurations. Of 315 headline sign comparisons,
259 agree in sign, 41 fall below the materiality floor on both coordinates, and
10 are material sign flips, a raw sign-agreement rate of 0.9365. Rank order is not
...
thousandths reorders configurations that were already close to tied. Everything in this subsection
that carries a margin above the frozen floors survives the change of coordinate; the fine ordering
of near-ties does not. The seven chronological folds are overlapping windows rather than independent
...
determinism controls engaged reproduces the unpinned ladder cell for cell, with the same largest
absolute log difference of 2.97e-01 under both, so pinning does not remove the difference and
