...
\paragraph{\latesttext{Secondary path summaries.}}
\latesttext{The full regularization paths are the primary empirical objects. For operational interpretability, we additionally report (i) a CV-only candidate-region screen with a soft activity marker and a family-specific upper deterioration/caution guardrail and (ii) a descriptive envelope of five prediction/COD turning events. Neither construction is an estimator, confidence interval, safe range, or model-selection rule. Both are assessed by leave-one-fold-out recomputation; their held-out and 2025 comparisons are retrospective because later-path information had already been examined during manuscript development. Appendix~\ref{app:ccao_path_details} states the algorithms, endpoints, stability checks, and value-regret calculations in full. Keeping those details in the appendix prevents a post-path screening device from being mistaken for the central method.}
