...
{We use chronological validation to characterize each complete regularization path; no configuration is ranked and no penalty weight is selected.} The framework is intentionally targeted: it does not, by itself, determine overall valuation level, eliminate nonlinear or local inequity, or replace the assessor-facing vertical-equity diagnostics used for evaluation.}
...
\newtext{Holding the surrounding model specification fixed makes the training-objective change as controlled as possible. The native-versus-custom $\rho=0$ comparison is reported in Appendix~\ref{app:implementation}; because the completed parity audit identifies this difference as an execution-path distinction, attribution of positive-penalty changes is made within each custom-objective family.}}
...
\latesttext{We also report the chronological-fold behaviour of those paths, together with the temporal failure-mode audit of Section~\ref{subsec:path_stability_results}. These characterize the paths and define no selected or recommended range.}
...
\newtext{Along these paths the broader distance-correlation diagnostic is quoted only at the zero-penalty origin, where it is $0.382$ on the held-out sample and $0.417$ in 2025. The frozen artifact that reproduces the positive-$\rho$ path anchors carries no distance-correlation column either, so no positive-$\rho$ path-anchor value and no claim about the shape of the dCor path are asserted for these paths. Supported positive-$\rho$ values of both residual-structure diagnostics do exist at the matched first-order and crossover coordinates, which rest on separate frozen artifacts.} 
...
\newtext{Second, stronger regularization exposes the limits of a global first-order target. The Surrogate provides the clearest current CCAO example: $\beta_{\log}$ and grouped diagnostics can approach neutrality while the ratio-shape profile remains non-monotone. At matched first-order correction the families also separate on non-affine residual structure rather than on the first-order coordinate alone (Section~\ref{subsec:matched_beta_results}). The methodological limitation is broader than that example, however: neither Direct nor Surrogate directly controls the non-affine conditional-mean structure measured by $\Delta_{\mathrm{NL}}$. Global first-order neutrality is therefore informative but not sufficient evidence of a flat value-related profile, broader residual--price independence, or locally uniform vertical equity.}
...
% Superseded limitations paragraph retained in the repository diff.
\latesttext{Third, the experiment characterizes fixed-994-tree regularization paths, not a deployment decision or fully retuned family frontier. Apart from selected group-profile intervals, the paper reports descriptive point estimates and fold variation rather than inferential uncertainty for path differences. The chronological split is ordered by sale date, but a final robustness audit should keep equal-date transactions together and block repeated sales of the same parcel to rule out boundary and parcel-history leakage. Descriptive development-period path landmarks were examined, but no stable penalty interval was identified: fold-level agreement is not a confidence interval, and the weak transfer of turning locations to the out-of-time blocks rules out treating raw-$\rho$ endpoints as portable. Finally, the closest post-processing comparator---the centered spread map implied by the Direct fixed-space result---is included rather than omitted, and it reproduces much of the Direct first-order movement over the range Direct attains, so this paper claims neither necessity nor general superiority for training-time regularization.}
...
\latesttext{For any future deployment-oriented calibration, the selection objective and its acceptance criteria should be frozen before evaluating later assessment cycles, and temporal stability should be reported across market periods rather than inferred from a single development-period landmark.}
...
\caption{\latesttext{Predictive-metric and valuation-level/uniformity paths along the Direct and Surrogate grids, with held-out and 2025 evaluations overlaid. Valuation-level panels include the ratio $=1$ reference. No penalty strength is selected in this paper.}}
...
\caption{\latesttext{Vertical-equity metric paths versus $\rho$ for Direct and Surrogate. Reference lines mark PRD $=1$, PRB $=0$, MKI $=1$, and VEI $=0$. No penalty strength is selected in this paper.}}
