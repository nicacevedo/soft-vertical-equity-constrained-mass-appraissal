...
\caption{\latesttext{Mechanism and residual-structure paths versus $\rho$ for Direct and Surrogate on held-out (solid) and 2025 (dashed) evaluations. The $\beta_{\log}=0$ line is a first-order neutrality reference; $\Delta_{\mathrm{NL}}$ and dCor are not given assessor-style zero targets. The shaded band and its dotted boundaries are a descriptive development-period landmark carried over from the frozen figure; they do not delimit a selected, portable, or recommended penalty interval, and no penalty strength is selected in this paper.}}
...
\caption{\latesttext{Vertical-equity metric paths versus $\rho$ for Direct and Surrogate. Reference lines mark PRD $=1$, PRB $=0$, MKI $=1$, and VEI $=0$. The shaded band and its dotted boundaries are a descriptive development-period landmark carried over from the frozen figure; they do not delimit a selected, portable, or recommended penalty interval, and no penalty strength is selected in this paper.}}
