...
\todo{Confirm once against the executed metric code that both PRB and VEI use Eq.~\eqref{eq:market_value_proxy} exactly; if not, fix the code and regenerate all affected results together.}
...
\begin{figure}[!htbp]
\centering
\safeincludegraphics[width=0.8\textwidth]{img/generated_v6_preselection/baseline_models_motivation_2024_2025.pdf}
...
Direct & 100 & 0.873 & \$89,309 & \textbf{1.036} & \textbf{-0.029} & \textbf{0.983} & \textbf{-4.5\%} & \textbf{-0.093} & \textbf{0.119} & 0.281 \\
\addlinespace[3pt]
\multicolumn{11}{l}{\textbf{Surrogate penalty}} \\
\cmidrule(lr){1-11}
...
\todo{Regenerate the primary accuracy--equity figure from the same frozen path table with only Linear, ordinary LightGBM, Direct, and Surrogate; remove the centered-recalibration curve from the plotting script and companion appendix figure.}
\begin{figure}[!htbp]
\centering
\safeincludegraphics[width=0.8\textwidth]{img/generated_v6_preselection/accuracy_equity_trajectories.pdf}
% {img/generated_v6_preselection/accuracy_equity_trajectories_inprocessing_only.pdf}
\caption{\newtext{Accuracy--equity trajectories for PRD and VEI against $R^2_P$ for the Direct and Surrogate regularization paths. Shaded regions are reference bands, not compliance bands. Linear and ordinary LightGBM are context anchors. Arrows indicate the direction of increasing $\rho$ and do not mark a selected point.}}
...
\newtext{the relevant empirical object before selecting an operating point}.
...
\todo{Regenerate the companion PRB/MKI accuracy--equity figure from the frozen path table with only Linear, ordinary LightGBM, Direct, and Surrogate.}
\begin{figure}[!htbp]
\centering
\safeincludegraphics[width=0.7\textwidth]{img/generated_v6_preselection/prb_mki_accuracy_equity.pdf}
% {img/generated_v6_preselection/prb_mki_accuracy_equity_inprocessing_only.pdf}
\caption{\newtext{Companion accuracy--equity trajectories for PRB and MKI against $R^2_P$ on the held-out and 2025 samples for the Direct and Surrogate regularization paths. The shaded PRB interval is the IAAO reference band $[-0.05,0.05]$; the dotted line marks MKI $=1$.}}
...
\begin{figure}[!htbp]
\centering
\safeincludegraphics[width=0.7\textwidth]{img/generated_v6_preselection/predictive_metric_paths.pdf}\\[2mm]
\safeincludegraphics[width=0.7\textwidth]{img/generated_v6_preselection/level_uniformity_paths.pdf}
