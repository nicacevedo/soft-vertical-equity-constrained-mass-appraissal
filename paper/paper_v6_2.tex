...
Working with data scientists and assessors at the Cook County Assessor's Office (CCAO), we study whether this application-specific pattern can be reduced through the model-training objective without replacing the surrounding data, features, validation, or review workflow. We use the first-order association between log valuation errors and log sale prices as a training regularization mechanism. We consider a direct squared-covariance penalty and develop a sample-additive upper-bound surrogate that is compatible with standard boosting implementations. \oldtext{The correction is intentionally targeted: it controls a global first-order sale-price-related pattern but does not determine overall valuation level, remove nonlinear or local inequity, or guarantee that assessor-facing vertical-equity measures fall within their reference ranges.}
\newtext{The correction is intentionally targeted at a global first-order sale-price-related pattern: it does not by itself determine overall valuation level, directly target nonlinear or local inequity, or guarantee that assessor-facing vertical-equity measures fall within their reference ranges.} We evaluate the correction using chronological validation, held-out and forward evaluation, complementary ratio-study and dependence diagnostics, and value-group ratio profiles. 
...
{\color{impGreen}
Second, because $\beta_{\log}$ measures only the best affine residual--price trend, we separately quantify nonlinear departures of the conditional mean relationship. Let
\[
Z=\frac{y-\E[y]}{\sqrt{\Var(y)}},
\qquad
m(z)=\E[e\mid Z=z],
\]
under the evaluation distribution, and let $\Pi_{\mathrm{aff}}m$ denote the
$L^2(P_Z)$ projection of $m$ onto the affine span of $\{1,Z\}$. We define the
\emph{normalized nonlinear lack-of-fit}
\begin{equation}
\label{eq:nonlinear_lack_of_fit}
\Delta_{\mathrm{NL}}
=
\frac{
\E\!\left[
...
\]
is the correlation ratio, or nonparametric coefficient of determination. Thus
$\Delta_{\mathrm{NL}}=0$ whenever the conditional mean of the log valuation
error is affine in log sale price, regardless of the slope of that affine
relationship, while positive values measure the additional conditional-mean
structure that cannot be represented by a straight line. The construction is
closely related to established correlation-ratio measures of regression
nonlinearity and to integrated squared lack-of-fit comparisons between
parametric and nonparametric regression curves
\cite{DoksumSamarov1995,HardleMammen1993}. This distinction is especially
relevant along a regularization path: $\lvert\beta_{\log}\rvert$ may continue
to fall even if stronger regularization begins to replace the targeted
first-order trend with an S-shaped, U-shaped, or otherwise nonlinear
residual--price pattern. We therefore use $\Delta_{\mathrm{NL}}$ as a
supplemental shape diagnostic, with zero as the idealized affine benchmark but
with no assessor-standard reference range.
}

\todo[inline]{\textcolor{impGreen}{Before finalizing the empirical results, state the exact cross-fitted smooth-regression estimator used for $\Delta_{\mathrm{NL}}$ (basis/smoother, folds, and smoothing rule). Fix this estimator before comparing penalty families or $\rho$ values, and use the same rule for every model within an evaluation split.}}

% \oldtext{Second, we report the \emph{distance correlation}, denoted by}
{\color{impGreen}Third, we report the \emph{distance correlation}, denoted by}
\begin{equation}
...
\end{equation}
% \oldtext{as an omnibus diagnostic of dependence between log valuation errors and log sale prices \cite{SzekelyRizzoBakirov2007}. Unlike the signed first-order slope, distance correlation can detect nonlinear and other non-first-order dependence but does not identify a regressive or progressive direction. Moreover, distributional independence is stronger than vertical neutrality as conventionally evaluated in ratio studies. We therefore use $\operatorname{dCor}(e,y)$ only as a supplemental diagnostic of remaining valuation-error--sale-price dependence, not as an assessor-facing measure of vertical equity.}
{\color{impGreen}as an omnibus diagnostic of dependence between log valuation errors and log sale prices \cite{SzekelyRizzoBakirov2007}. Unlike $\Delta_{\mathrm{NL}}$, which isolates departures of the conditional mean from its best affine representation, distance correlation can respond to broader forms of distributional dependence, including nonlinear mean relationships, price-related dispersion, and other non-first-order structure. It does not identify a regressive or progressive direction, and distributional independence is stronger than vertical neutrality as conventionally evaluated in ratio studies. We therefore use $\operatorname{dCor}(e,y)$ only as a supplemental diagnostic of remaining valuation-error--sale-price dependence, not as an assessor-facing measure of vertical equity.}
...
\newtext{\baselinemetric{$\Delta_{\mathrm{NL}}$}} & \newtext{\textit{pending}} & \newtext{\textit{pending}} & \newtext{\textit{pending}} & \newtext{\textit{pending}} \\
...
382,897 eligible 2016--2024 sales. \oldtext{The supplemental slope $\beta_{\log}$ and distance correlation are defined in Eqs.~\eqref{eq:log_ratio_slope} and~\eqref{eq:dcor_diagnostic}, respectively.}
\newtext{The supplemental slope $\beta_{\log}$, nonlinear lack-of-fit $\Delta_{\mathrm{NL}}$, and distance correlation are defined in Eqs.~\eqref{eq:log_ratio_slope}, \eqref{eq:nonlinear_lack_of_fit}, and~\eqref{eq:dcor_diagnostic}, respectively. The $\Delta_{\mathrm{NL}}$ cells are placeholders to be populated from the same frozen baseline prediction artifacts used for the other diagnostics.} Boldface indicates the preferred
...
\begin{figure}[!htbp]
\centering
\todo[inline]{\textcolor{impGreen}{Regenerate Figure~\ref{fig:baseline_motivation} from the same frozen baseline predictions after the local-results update. Remove the generic raw-price ``Slope'' annotation and retain only the defined $\beta_{\log}$ mechanism annotation; use common axis limits across the four panels; choose the common ratio-axis range from the displayed profiles so no median/IQR is clipped; and keep a consistent mathematical log-scale tick formatter across all sale-price profile figures.}}
...
{\color{impGreen}Neither formulation directly optimizes PRD, PRB, VEI, COD, or assessment level. Nor does zero training covariance imply the absence of nonlinear, geographic, property-class, temporal, or other local value-related patterns. The training covariance is therefore treated as a mechanism target rather than as a sufficient measure of vertical equity. Predictive performance, valuation level, PRD, PRB, VEI, ratio curves, the nonlinear lack-of-fit diagnostic $\Delta_{\mathrm{NL}}$ in~\eqref{eq:nonlinear_lack_of_fit}, and the broader distance-correlation diagnostic remain separate validation and out-of-sample evaluation criteria; subgroup diagnostics remain necessary complements for later operational validation.}
...
% \oldtext{A validation-fitted one-parameter rescaling of the baseline predictions is included as a low-cost post-hoc competitor where reported in the Results, particularly in the multi-county benchmark. This comparison is necessary because a global spread correction may reproduce part of the penalty's effect without modifying model training; improvement relative to standard LightGBM alone is therefore not evidence that in-processing regularization is necessary.}
\newtext{The exploratory multi-county benchmark includes a validation-fitted centered rescaling of the baseline predictions. We treat the same one-parameter family as the relevant low-cost comparator for the final primary CCAO analysis because a global spread correction may reproduce part of the first-order effect without modifying model training; improvement relative to standard LightGBM alone is therefore not evidence that in-processing regularization is necessary.}

% \oldtext{A constant multiplicative level adjustment and a slope-changing rescaling serve different roles. Multiplying every price prediction by a constant adds a constant to every log residual and therefore leaves $\operatorname{Cov}(e,y)$ unchanged. By contrast, a one-degree calibration that changes the spread of predictions can alter the ratio--value slope and is the relevant competitor in the multi-county benchmark.}
\newtext{For a baseline log-price predictor $f_0$ and a training block $\mathcal T$, define the centered one-parameter family
\begin{equation}
\label{eq:centered_recalibration}
f_b(x)
=
\bar y_{\mathcal T}
+
b\bigl(f_0(x)-\bar y_{\mathcal T}\bigr),
\qquad b>0,
\end{equation}
where $\bar y_{\mathcal T}$ is computed from training data only and $b=1$ recovers the baseline predictor. This spread-changing map can alter the log-ratio--log-price slope. By contrast, multiplying every price prediction by a common positive constant only adds a constant to every log residual and leaves $\operatorname{Cov}(e,y)$ unchanged.}

\todo[inline]{\textcolor{impGreen}{Before final submission, add the centered-recalibration comparator in Eq.~\eqref{eq:centered_recalibration} to the primary CCAO experiment under the same chronological protocol. Determine any path/grid or fitted value of $b$ using development/training-validation information only; do not use held-out or 2025 outcomes to choose it. Until this comparison is displayed, do not claim that in-processing regularization adds value beyond simple spread recalibration in the primary CCAO application.}}
...
\todo[inline]{\textcolor{impGreen}{Final local-results gate for this section: (i) resolve the split-specific native/custom $\rho=0$ control; (ii) compute the fixed-estimator $\Delta_{\mathrm{NL}}$ for baselines, every chronological fold, every held-out/2025 Direct/Surrogate path point, and centered recalibration; (iii) run the primary CCAO centered-recalibration benchmark using development information only; and (iv) regenerate the affected tables/figures with the axis/unit corrections specified below. These are the only new primary CCAO result computations required by the present pre-selection revision. Do not add matched-$s$ experiments, demographic subgroup sweeps, extra dependence metrics, post-hoc red/green ``safety'' bands, curve blurring, or exhaustive pairwise tradeoff matrices unless a later claim specifically requires them.}}
...
\todo[inline]{\textcolor{impGreen}{Local validity gate: recompute the native-versus-custom $\rho=0$ control independently for the held-out and 2025 forward prediction artifacts, aligned on sale identifiers. Do not reuse held-out mean/max prediction-deviation summaries in the forward rows. Verify the native/custom fits under the identical frozen 994-tree parameter vector, seed, categorical handling, initialization, objective normalization, and training data. Then regenerate Table~\ref{tab:rho_zero_control} and this subsection from those split-specific artifacts. If numerical parity is recovered, report it; otherwise retain the custom-$\rho=0$ fit as the within-family reference and document the reproducible implementation-path cause. Positive-$\rho$ sweeps need not be rerun unless this audit reveals an error in the penalized objective itself.}}
...
\todo[inline]{\textcolor{impGreen}{Compute $\Delta_{\mathrm{NL}}$ from the frozen row-level prediction artifacts using one estimator specification fixed before family/path comparison. Populate the pending $\Delta_{\mathrm{NL}}$ column below for Linear, ordinary LightGBM, both custom $\rho=0$ controls, and every prespecified Direct/Surrogate display anchor on held-out and 2025. The same metric must also be written for every full-grid path point and chronological fold in the machine-readable artifacts. Zero is the affine benchmark; there is no external acceptance band, and anchors must not be selected or omitted on the basis of this diagnostic.}}
...
& \$90,092
& \textbf{1.042}
& \textbf{-0.016}\textsuperscript{*}
& \textbf{0.975}\textsuperscript{*}
& \textbf{-11.6\%}
& \textbf{-0.092}
& \newtext{\textit{pending}}
& \textbf{0.250} \\
...
& \textbf{\$75,218}
& \textbf{1.036}
& -0.040\textsuperscript{*}
& \textbf{0.984}\textsuperscript{*}
& \textbf{-10.5\%}
& -0.096
& \newtext{\textit{pending}}
& 0.302 \\
...
& 0.869
& \$84,918
& \textbf{1.029}\textsuperscript{*}
& \textbf{-0.015}\textsuperscript{*}
& \textbf{0.998}\textsuperscript{*}
& \textbf{0.7\%}\textsuperscript{*}
& \textbf{-0.079}
& \newtext{\textit{pending}}
& 0.265 \\
...
& \textbf{\$75,468}
& 1.049
& -0.045\textsuperscript{*}
& 0.953\textsuperscript{*}
& -12.6\%
& -0.102
& \newtext{\textit{pending}}
& 0.310 \\
...
& 0.893
& \$78,110
& \textbf{1.024}\textsuperscript{*}
& \textbf{0.013}\textsuperscript{*}
& \textbf{0.988}\textsuperscript{*}
& \textbf{0.2\%}\textsuperscript{*}
& \textbf{-0.036}
& \newtext{\textit{pending}}
& \textbf{0.250} \\
...
& 0.889
& \$82,094
& \textbf{1.010}\textsuperscript{*}
& 0.042\textsuperscript{*}
& \textbf{1.013}\textsuperscript{*}
& \textbf{4.0\%}\textsuperscript{*}
& \textbf{-0.001}
& \newtext{\textit{pending}}
& 0.267 \\
...
& \$99,371
& \textbf{1.052}
& \textbf{-0.029}\textsuperscript{*}
& \textbf{0.954}\textsuperscript{*}
& \textbf{-17.3\%}
& \textbf{-0.109}
& \newtext{\textit{pending}}
& \textbf{0.269} \\
...
& \$79,187
& \textbf{1.042}
& -0.049\textsuperscript{*}
& \textbf{0.971}\textsuperscript{*}
& \textbf{-11.9\%}
& \textbf{-0.105}
& \newtext{\textit{pending}}
& 0.318 \\
...
& \textbf{1.036}
& \textbf{-0.029}\textsuperscript{*}
& \textbf{0.983}\textsuperscript{*}
& \textbf{-4.5\%}\textsuperscript{*}
& \textbf{-0.093}
& \newtext{\textit{pending}}
& 0.281 \\
...
& \$81,075
& \textbf{1.033}
& \textbf{0.000}\textsuperscript{*}
& \textbf{0.967}\textsuperscript{*}
& \textbf{-2.0\%}\textsuperscript{*}
& \textbf{-0.050}
& \newtext{\textit{pending}}
& \textbf{0.258} \\
...
& 0.900
& \$85,257
& \textbf{1.020}\textsuperscript{*}
& 0.030\textsuperscript{*}
& \textbf{0.987}\textsuperscript{*}
& \textbf{1.7\%}\textsuperscript{*}
& \textbf{-0.015}
& \newtext{\textit{pending}}
& \textbf{0.266} \\
...
MKI $[0.95,1.05]$, and VEI $[-10\%,10\%]$. Higher $R^2_P$ and lower MAE and
\oldtext{dCor are preferred; PRD and MKI are compared by distance from one, while PRB, VEI, and $\beta_{\log}$ are compared by distance from zero.}
\newtext{dCor and $\Delta_{\mathrm{NL}}$ are preferred when lower; PRD and MKI are compared by distance from one, while PRB, VEI, and $\beta_{\log}$ are compared by distance from zero. Pending $\Delta_{\mathrm{NL}}$ entries are not used for boldface until populated.} Boldface and
asterisks do not denote statistical significance, formal compliance, or model
selection.
\end{minipage}
...
\begin{figure}[!htbp]
\centering
\todo[inline]{\textcolor{impGreen}{Regenerate this figure from the unchanged prespecified display anchors using one common x-range, one common y-range, and identical sale-price log ticks across all four panels. Choose the shared y-range from the complete displayed profiles rather than a fixed hard-coded bound so no low-price median curve is clipped. Do not change opacity, omit curves, or visually de-emphasize anchors on the basis of $\Delta_{\mathrm{NL}}$ or dCor outcomes.}}
...
\newtext{At the prespecified anchors currently displayed, the Direct profile changes more smoothly than the Surrogate profile.} Under stronger Surrogate regularization, the profile
...
\todo[inline]{\textcolor{impGreen}{Insert the $\Delta_{\mathrm{NL}}$ path result here once computed. The key descriptive comparison is whether the nonlinear lack-of-fit remains flat/decreases through the moderate-$\rho$ region and then rises at stronger regularization, and whether any rise coincides with the non-monotone ratio profiles above. Report Direct and Surrogate separately and avoid labeling the departure specifically as ``S-shaped'' unless the plotted conditional-mean shape supports that description.}}
...
{\color{impGreen}The direct training target is covariance, and $\beta_{\log}$ is its signed first-order slope equivalent up to the fixed variance of log sale price within an evaluation sample. The nonlinear lack-of-fit $\Delta_{\mathrm{NL}}$ complements this first-order mechanism by isolating conditional-mean structure that remains after the best affine residual--price trend is removed, while distance correlation provides a broader omnibus check for remaining distributional dependence. These diagnostics answer distinct questions and are therefore traced along the complete paths rather than evaluated only at a single positive-$\rho$ model.}

\todo[inline]{\textcolor{impGreen}{Regenerate Figure~\ref{fig:mechanism_path_placeholder} as a $3\times2$ mechanism/shape figure: rows $\beta_{\log}$, $\Delta_{\mathrm{NL}}$, and dCor; columns Direct and Surrogate. Use the complete grids, show the $\rho=0$ control explicitly, keep held-out/2025 as line styles, and use identical y-limits within each metric row across the two families. Keep the $\Delta_{\mathrm{NL}}$ estimator fixed across every path point. Use a zero reference line for $\beta_{\log}$ and the affine benchmark $0$ for $\Delta_{\mathrm{NL}}$; do not invent a red/green acceptance band for $\Delta_{\mathrm{NL}}$ or dCor because neither has an assessor-standard cutoff.}}

\begin{figure}[!htbp]
\centering
\safeincludegraphics[width=0.8\textwidth]{img/generated_v6_preselection/mechanism_vs_rho.pdf}
\caption{\oldtext{First-order slope $\beta_{\log}$ and distance correlation along the full Direct and Surrogate grids. The $\rho=0$ control is shown explicitly; positive $\rho$ is on a log scale. Held-out and 2025 paths use distinct line styles.} \newtext{First-order slope $\beta_{\log}$, nonlinear conditional-mean lack-of-fit $\Delta_{\mathrm{NL}}$, and distance correlation along the full Direct and Surrogate grids. The $\rho=0$ control is shown explicitly; positive $\rho$ is on a log scale. Held-out and 2025 paths use distinct line styles. Within each metric row, Direct and Surrogate use the same vertical scale.}}
...
\todo[inline]{\textcolor{impGreen}{Add the corresponding $\Delta_{\mathrm{NL}}$ results here. Highest-value evidence: identify whether $\lvert\beta_{\log}\rvert$ continues to decrease while $\Delta_{\mathrm{NL}}$ begins to increase at stronger $\rho$. Report the turning behavior, if present, for both Direct and Surrogate and for both held-out and 2025 evaluations. Treat dCor as the broader dependence diagnostic and $\Delta_{\mathrm{NL}}$ as the conditional-mean shape diagnostic; do not conflate the two.}}
...
\begin{figure}[!htbp]
\centering
\todo[inline]{\textcolor{impGreen}{Regenerate the PRD/VEI accuracy--equity figure from the frozen paths after the primary CCAO centered-recalibration benchmark is computed. Use a common $R^2_P$ range across all panels and a common vertical range wherever the same fairness metric is repeated across held-out/2025 panels. Overlay the validation-fitted centered-recalibration result as a distinct neutral marker on held-out and 2025 panels; do not connect it to the $\rho$ paths or imply it is a penalty-path point. Keep the existing reference-band language as descriptive rather than compliance-oriented.}}
...
\midrule
Cook, IL & 0.833 & \oldtext{0.283}\newtext{0.282} & \oldtext{20.7}\newtext{20.9} & \oldtext{21.199}\newtext{21.266} & \oldtext{1.083}\newtext{1.073} & \oldtext{-0.098}\newtext{-0.079} & \oldtext{0.959}\newtext{0.976} \\
Allegheny, PA & \oldtext{0.779}\newtext{0.781} & \oldtext{0.325}\newtext{0.324} & \newtext{25.3} & \oldtext{26.129}\newtext{26.061} & 1.103 & \oldtext{-0.101}\newtext{-0.103} & \oldtext{0.954}\newtext{0.956} \\
Maricopa, AZ & \oldtext{0.844}\newtext{0.848} & \oldtext{0.222}\newtext{0.219} & \oldtext{14.7}\newtext{14.0} & \oldtext{14.797}\newtext{14.171} & \oldtext{1.072}\newtext{1.076} & \oldtext{-0.055}\newtext{-0.060} & \oldtext{0.989}\newtext{0.977} \\
King, WA & \oldtext{0.798}\newtext{0.814} & \oldtext{0.261}\newtext{0.250} & \oldtext{20.7}\newtext{16.7} & \oldtext{16.336}\newtext{16.790} & \oldtext{1.073}\newtext{1.102} & \oldtext{-0.028}\newtext{-0.074} & \oldtext{1.106}\newtext{0.948} \\
Miami-Dade, FL & \oldtext{0.808}\newtext{0.815} & \oldtext{0.316}\newtext{0.310} & \oldtext{26.3}\newtext{25.4} & \oldtext{24.210}\newtext{24.181} & \oldtext{1.092}\newtext{1.086} & \oldtext{0.025}\newtext{0.027} & \oldtext{1.060}\newtext{1.039} \\
Middlesex, MA & \oldtext{0.780}\newtext{0.790} & \oldtext{0.264}\newtext{0.258} & \oldtext{19.1}\newtext{18.6} & \oldtext{18.565}\newtext{18.255} & \oldtext{1.085}\newtext{1.083} & \oldtext{-0.096}\newtext{-0.088} & \oldtext{1.023}\newtext{1.014} \\
\bottomrule
\end{tabular}}
\end{table}

\todo[inline]{Keep Table~\ref{tab:attom_baselines}, Table~\ref{tab:attom_selected_penalty}, and Appendix Table~\ref{tab:attom_design_appendix} generated from the same frozen county-report artifact. If the local benchmark is regenerated, update all three together rather than manually mixing benchmark snapshots.}
...
\newtext{These results do not establish a universal or confirmatory compliance result and do not imply that the selected penalty dominates simpler post-hoc recalibration; the latter comparison is discussed next.}

\subsection{ \newtext{Recalibration Remains a Required Benchmark}}
\label{subsec:scaling_results}

% \oldtext{The multi-county benchmark includes validation-fitted centered recalibration maps of the baseline log predictions. These comparators can change the spread of predictions after model fitting and can therefore alter a global ratio--value trend. A constant multiplicative level adjustment is different: it shifts every log residual by the same amount and leaves the targeted covariance unchanged.}
\newtext{The exploratory multi-county benchmark includes validation-fitted centered recalibration maps of the baseline log predictions, as defined in Eq.~\eqref{eq:centered_recalibration}. These maps can change the spread of predictions after model fitting and therefore alter a global ratio--value trend; a constant multiplicative level adjustment cannot.}

% \oldtext{The existing comparisons do not support a general claim that the custom objective dominates the simpler recalibration. The in-processing penalty may still be useful when the target must influence tree construction or when the method will be extended to geographic, class-specific, or other structured constraints.}
\newtext{These exploratory comparisons do not support a general claim that the custom objective dominates the simpler recalibration. They also do not yet identify the incremental value of in-processing regularization in the primary CCAO application because the corresponding centered-recalibration path is not yet displayed there. The in-processing penalty may still be useful when changing the training objective produces out-of-time gains that cannot be reproduced by this simpler benchmark, or when future extensions impose structured constraints during model fitting.}
\newtext{For the current paper, the defensible comparison is to report both approaches using only training and validation information and to treat simple recalibration as a benchmark for the incremental value of in-processing regularization.}

\todo[inline]{\textcolor{impGreen}{Run the primary CCAO centered-recalibration benchmark in Eq.~\eqref{eq:centered_recalibration} under the same chronological design. Fit/tune $b$ using development/rolling-validation information only; then refit the baseline on the same full development samples used by the main experiment and apply the frozen recalibration to held-out and 2025. Populate Table~\ref{tab:ccao_recalibration_placeholder} and overlay the same result in the accuracy--equity figures. Save the complete validation record for $b$ and the row-level recalibrated predictions. Do not use held-out or 2025 outcomes to choose $b$, and do not select a Direct/Surrogate $\rho$ merely to make the comparison favorable.}}
...
\scriptsize
\renewcommand{\arraystretch}{1.06}
\caption{Primary CCAO centered-recalibration benchmark. The recalibration parameter is determined using development/chronological-validation information only; held-out and 2025 values are reported after the choice is frozen.}
...
\midrule
Held-out & Ordinary LightGBM & 1 & 0.894 & \$75,655 & 1.069 & -0.091 & 0.923 & -26.5\% & -0.150 & \textit{pending} & 0.387 \\
Held-out & Centered recalibration & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} \\
\addlinespace
2025 forward & Ordinary LightGBM & 1 & 0.904 & \$78,484 & 1.079 & -0.106 & 0.907 & -28.6\% & -0.164 & \textit{pending} & 0.422 \\
2025 forward & Centered recalibration & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} & \textit{pending} \\
\bottomrule
\end{tabular}}
\vspace{1mm}
\begin{minipage}{0.98\textwidth}
\scriptsize
\emph{Notes.} The table is a local-pipeline placeholder. The final version should be generated from the frozen baseline and recalibrated prediction artifacts, with the same metric implementations used elsewhere in the primary CCAO analysis. $\Delta_{\mathrm{NL}}$ uses the fixed estimator specified with Eq.~\eqref{eq:nonlinear_lack_of_fit}. This table does not select or rank a penalized $\rho$.
\end{minipage}
\end{table}
}

\todo{After Table~\ref{tab:ccao_recalibration_placeholder} is populated, rewrite the surrounding comparison using only what the held-out/2025 evidence supports. If centered recalibration reproduces the relevant path improvements, state that directly; if in-processing provides incremental gains, identify the specific dimensions and evaluation periods. Otherwise move any stronger comparative interpretation to Discussion/Limitations.}
...
\newtext{$\Delta_{\mathrm{NL}}$, ratio-shape, and stratified/local diagnostics} & \newtext{Separate nonlinear conditional-mean departures from the first-order trend and detect offsetting geographic or class-specific patterns hidden by aggregate measures} \\
Temporal validation and uncertainty analysis & Test whether regularization-path behavior generalizes across market periods \\
\bottomrule
\end{tabular}
\end{table}
\todo[inline]{\oldtext{We should still add the spatial/demographic/other consequences, but I am not sure how much space we have.} \newtext{Scope decision for the present pre-selection paper: do not add a new race/socioeconomic/subgroup penalty-path campaign unless a later paper claim specifically requires it. Geographic/property-class subgroup diagnostics remain an operational-validation and future-work requirement, already acknowledged in this section and in the limitations, rather than a missing headline result.}}
...
\newtext{Second, stronger regularization exposes the limits of a global first-order target. The Surrogate provides the clearest current CCAO example: $\beta_{\log}$ and grouped diagnostics can approach neutrality while the ratio-shape profile remains non-monotone and distance correlation rebounds. The methodological limitation is broader than that example, however: neither Direct nor Surrogate directly controls nonlinear conditional-mean lack-of-fit. Global first-order neutrality is therefore informative but not sufficient evidence of a flat value-related profile, broader residual--price independence, or locally uniform vertical equity.}
...
\todo[inline]{\textcolor{impGreen}{Before freezing the final empirical artifacts, audit the exact executed code against the normalization in Eqs.~\eqref{eq:baseline_learning}, \eqref{eq:direct_penalty}, and \eqref{eq:surrogate_learning} and against the $n/2$ LightGBM scaling stated here. Record the exact code/configuration hash. Because the empirical paths are indexed by numerical values of $\rho$, any factor-of-two or sample-size convention must be reconciled explicitly rather than absorbed silently into a relabeled $\rho$. Combine this audit with resolution of the native-versus-custom $\rho=0$ discrepancy.}}
...
\todo[inline]{\oldtext{Appendix-only robustness note proposed for Direct-MM $K=1$ and QNL after recomputing $\Delta_{\mathrm{NL}}$ on the toy paths.} \newtext{Scope decision: do not add Direct-MM $K=1$ or QNL to the primary CCAO Results merely because toy artifacts exist. They remain mechanism-development experiments. Add an appendix robustness note only if later claims about numerical Direct implementation or nonlinear guardrails require those experiments; otherwise no additional local toy run is required for the current Results revision.}}
...
\todo[inline]{\textcolor{impGreen}{Extend the machine-readable CCAO artifacts to include $\Delta_{\mathrm{NL}}$ for Linear, ordinary LightGBM, custom $\rho=0$, every Direct/Surrogate chronological fold and out-of-time path point, and the centered-recalibration benchmark. Store the fixed $\Delta_{\mathrm{NL}}$ estimator specification (fold assignment rule, smoother/basis, tuning rule, seed if any) alongside the values. Also save the split-specific $\rho=0$ prediction-parity audit and the validation record for the recalibration parameter so every paper table/figure can be regenerated from the same frozen artifact set.}}

\subsection{Companion PRB and MKI Accuracy--Equity Paths}

\begin{figure}[!htbp]
\centering
\todo[inline]{\textcolor{impGreen}{Regenerate this figure with an in-figure Direct/Surrogate legend, human-readable panel titles, the PRB reference band $[-0.05,0.05]$, and the MKI neutral reference line at 1. Use common $R^2_P$ limits across panels and common vertical limits whenever the same metric is repeated across held-out/2025. Overlay the frozen primary CCAO centered-recalibration result as a distinct unconnected marker after it is computed.}}
...
\begin{figure}[!htbp]
\centering
\todo[inline]{\textcolor{impGreen}{Regenerate the MAPE and COV panels in percentage units (MAPE (\%) and COV (\%)) to match Section~\ref{subsec:metrics} and Tables~\ref{tab:assessment_metrics_summary}--\ref{tab:ccao_baseline_results}. For every metric row, use the same y-limits in the Direct and Surrogate panels so visual differences are not created by rescaling. Keep the existing common $\rho$ ticks and held-out/2025 line-style convention.}}
...
\begin{figure}[!htbp]
\centering
\todo[inline]{\textcolor{impGreen}{When regenerating the chronological-stability artifact, enforce identical y-limits across Direct/Surrogate for each metric row. Compute and archive fold-level $\Delta_{\mathrm{NL}}$ on the same seven folds. Add a $\Delta_{\mathrm{NL}}$ row to this appendix figure only if it remains legible; otherwise keep the fold-level values in the machine-readable table and retain the current four-row figure.}}
...
\oldtext{\advisorchange{A replication package for the CCAO application should contain} the complete penalty grid, fold-level validation predictions, held-out and forward predictions for every valid configuration, the frozen baseline hyperparameters, and assessor metrics before and after any separate level calibration. It should include both the ordinary LightGBM baseline and a custom-objective fit with zero penalty.}
\newtext{A replication package for the CCAO application should contain the complete penalty grid; fold-level, held-out, and forward predictions for every valid configuration; the frozen baseline hyperparameters and configuration hash; the ordinary LightGBM and custom-objective $\rho=0$ prediction-parity audit for both out-of-time splits; the fixed $\Delta_{\mathrm{NL}}$ estimator specification and resulting values; and the complete primary centered-recalibration validation record and predictions.} Before operational use, the same analysis should report block-bootstrap intervals and ratio curves by township, property class, value range, and time. These outputs separate model-selection uncertainty, implementation effects, and local variation.\todo{Before submission, verify which listed CCAO artifacts are included in the released replication package. If all are archived, replace the prospective ``should contain'' wording above with an assertive description of the released materials.}
...
\newtext{The benchmark compares ordinary LightGBM, the custom-objective zero-penalty control, positive \texttt{LGBCovPenalty[diff]} values, and the centered one-parameter recalibration family in Eq.~\eqref{eq:centered_recalibration}. County-specific feature sets, LightGBM configurations, penalty strengths, and recalibration strengths are chosen using development/chronological-validation information only. The benchmark applies the same Pareto--Nash maximum-volume-hyperrectangle rule over MAPE, COD, $|\mathrm{PRD}-1|$, and $|\mathrm{PRB}|$ to these validation-stage choices; the held-out block is evaluated only after the corresponding choice and final refit. Constant multiplicative level calibration is treated separately because it changes the overall ratio level but leaves the targeted log-residual covariance unchanged.}
