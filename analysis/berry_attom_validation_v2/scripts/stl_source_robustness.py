...
def common_key(frame: pd.DataFrame, apn_col: str, date_col: str, price_col: str) -> pd.Series:
    apn = normalize_apn_series(frame[apn_col])
    dt = pd.to_datetime(frame[date_col]).dt.strftime("%Y-%m-%d")
    price = pd.to_numeric(frame[price_col], errors="coerce").round(0).astype("Int64").astype(str)
    return apn + "|" + dt + "|" + price
...
def fit_lgbm(data: pd.DataFrame, features: pd.DataFrame, categorical: list[str], split: int, validation_split: int):
    ...
    model = LGBMRegressor(**params)
    model.fit(features.iloc[:split], y_log.iloc[:split], categorical_feature=categorical)
...
def main() -> int:
    ...
    local["common_key"] = common_key(local, "PARID", "sale_date", "sale_price")
    apn_col = "PARCELNUMBERFORMATTED" if "PARCELNUMBERFORMATTED" in attom.columns else "PARCELNUMBERRAW"
    attom["common_key"] = common_key(attom, apn_col, "sale_date", "sale_price")
    local_u = local.drop_duplicates("common_key", keep="first")
    attom_u = attom.drop_duplicates("common_key", keep="first")
    keys = sorted(set(local_u["common_key"]) & set(attom_u["common_key"]))
    ...
    if not (1 <= validation_split < split < n):
        raise ValueError(f"common sample too small: n={n}")
    ...
    attom_feat, attom_cat = feature_frame(attom_c, split, False, False)
    ...
    train_price = local_c.sale_price.iloc[:split].to_numpy()
    test = local_c.iloc[split:]
    rows = []
    for name, pred, cfg in (
        ("local_historical_dwelling", local_pred, local_cfg),
        ("attom_assessor_history", attom_pred, attom_cfg),
    ):
        mets = enrich(local_c.sale_price.iloc[split:].to_numpy(), pred[split:], train_price)
        _, summary = bootstrap_scores(test, pred[split:], train_price, N_BOOTSTRAP, "M", SEED)
        row = {"source": name, "selected_lgbm_config": cfg, "n_common": n, "n_test": int(len(test))}
        row.update({k: v for k, v in mets.items() if k != "ratio_by_decile"})
        row["ratio_by_decile"] = json.dumps(mets.get("ratio_by_decile"))
        rows.append(row)
        summary.assign(source=name).to_csv(OUT_A / f"bootstrap_{name}.csv", index=False)

    common = local_c[["common_key", "PARID", "sale_date", "sale_price"]].copy()
    common["pred_local"] = local_pred
    common["pred_attom"] = attom_pred
    common["split"] = np.where(np.arange(n) < split, "train", "test")
    ...
    axes[0].legend()
    axes[0].set_title("Held-out test metrics")
    for ax, src, pred in (
        (axes[1], "local", local_pred),
        (axes[2], "ATTOM", attom_pred),
    ):
        ratio = pred[split:] / local_c.sale_price.iloc[split:].to_numpy()
        q = pd.qcut(local_c.sale_price.iloc[split:], 10, labels=False, duplicates="drop")
    ...
    report = [
        ...
        "",
        f"Common N={n}; test N={int(len(test))}.",
        "",
