...
def fit_models(data: pd.DataFrame, label: str) -> pd.DataFrame:
    ...
    lgbm.fit(features.iloc[:split], y_log.iloc[:split], categorical_feature=CATEGORICAL)
    ...
    lr.fit(features.iloc[:split], y_log.iloc[:split])
    lr_pred = np.exp(lr.predict(features))
    train_price = data.sale_price.iloc[:split].to_numpy()
    test = data.iloc[split:]
    rows = []
    for model_name, pred in (("LinearRegression", lr_pred), ("LGBMRegressor", lgbm_pred)):
        test_m = enrich(data.sale_price.iloc[split:].to_numpy(), pred[split:], train_price)
        _, summary = bootstrap_scores(test.assign(sale_price=data.sale_price), pred[split:], train_price, N_BOOTSTRAP, "M", SEED)
        row = {"sample": label, "model": model_name, "n": int(len(data)), "n_test": int(len(test)),
               "selected_lgbm_config": selected if model_name == "LGBMRegressor" else "",
               **{k: v for k, v in test_m.items() if not isinstance(v, dict)}}
        rows.append(row)
        summary.assign(sample=label, model=model_name).to_csv(OUT_A / f"bootstrap_{label}_{model_name}.csv", index=False)
