...
#SBATCH --output=analysis/berry_attom_validation_v3/logs/surrogate_%A_%a.out
#SBATCH --error=analysis/berry_attom_validation_v3/logs/surrogate_%A_%a.err
set -euo pipefail
...
echo "surrogate county=${KEY} cpus=${CPUS} host=$(hostname) partition=${SLURM_JOB_PARTITION:-} start=$(date -Is)"
"${PY}" analysis/berry_attom_validation_v3/scripts/run_direct_surrogate.py --county-key "${KEY}" --path surrogate --lgbm-threads "${CPUS}"
