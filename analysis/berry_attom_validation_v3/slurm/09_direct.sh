...
#SBATCH --output=analysis/berry_attom_validation_v3/logs/direct_%A_%a.out
#SBATCH --error=analysis/berry_attom_validation_v3/logs/direct_%A_%a.err
set -euo pipefail
...
echo "direct county=${KEY} cpus=${CPUS} host=$(hostname) partition=${SLURM_JOB_PARTITION:-} start=$(date -Is)"
"${PY}" analysis/berry_attom_validation_v3/scripts/run_direct_surrogate.py --county-key "${KEY}" --path direct --lgbm-threads "${CPUS}"
