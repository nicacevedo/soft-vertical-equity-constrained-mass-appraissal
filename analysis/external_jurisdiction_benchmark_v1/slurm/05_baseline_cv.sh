...
#SBATCH --output=analysis/external_jurisdiction_benchmark_v1/logs/basecv_%A_%a.out
#SBATCH --error=analysis/external_jurisdiction_benchmark_v1/logs/basecv_%A_%a.err
set -euo pipefail
...
echo "basecv county=${KEY} cpus=${CPUS} host=$(hostname) partition=${SLURM_JOB_PARTITION:-} start=$(date -Is)"
