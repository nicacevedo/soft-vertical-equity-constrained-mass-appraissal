...
#SBATCH --job-name=v1_surrogate_panel
...
#SBATCH --output=analysis/external_jurisdiction_benchmark_v1/logs/surrogatepanel_%A_%a.out
#SBATCH --error=analysis/external_jurisdiction_benchmark_v1/logs/surrogatepanel_%A_%a.err
set -euo pipefail
...
echo "surrogate_panel county=${KEY} cpus=${CPUS} host=$(hostname) partition=${SLURM_JOB_PARTITION:-} start=$(date -Is)"
