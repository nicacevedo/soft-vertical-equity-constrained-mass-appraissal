...
#SBATCH --mem=80G
#SBATCH --time=4:00:00
#SBATCH --output=analysis/external_jurisdiction_benchmark_v1/logs/tables2_%A_%a.out
#SBATCH --error=analysis/external_jurisdiction_benchmark_v1/logs/tables2_%A_%a.err
set -euo pipefail
...
cd "${REPO_ROOT}"
KEYS=(wayne philadelphia st_louis_county maricopa king miami_dade middlesex)
KEY="${KEYS[${SLURM_ARRAY_TASK_ID:-0}]}"
CPUS="${SLURM_CPUS_PER_TASK:-8}"
export PYTHONUNBUFFERED=1 OMP_NUM_THREADS="${CPUS}"
echo "tables2 county=${KEY} cpus=${CPUS} host=$(hostname) partition=${SLURM_JOB_PARTITION:-} start=$(date -Is)"
