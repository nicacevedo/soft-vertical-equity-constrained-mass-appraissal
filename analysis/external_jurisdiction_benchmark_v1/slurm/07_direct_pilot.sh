#!/bin/bash
#SBATCH --job-name=v1_direct_pilot
#SBATCH --array=0-2
...
#SBATCH --output=analysis/external_jurisdiction_benchmark_v1/logs/directpilot_%A_%a.out
#SBATCH --error=analysis/external_jurisdiction_benchmark_v1/logs/directpilot_%A_%a.err
set -euo pipefail
...
echo "direct_pilot county=${KEY} cpus=${CPUS} host=$(hostname) partition=${SLURM_JOB_PARTITION:-} start=$(date -Is)"
